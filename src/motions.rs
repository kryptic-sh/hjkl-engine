//! Vim-shaped cursor motions, computed over [`hjkl_buffer::Buffer`].
//!
//! Patch C (0.0.30) relocates the 24 motion helpers from `hjkl-buffer`
...
//! `move_screen_down`) take a caller-owned `sticky_col`
//! (vim's `curswant`) so bouncing through a shorter row doesn't drag
//! the cursor back to col 0. Word motions (`move_word_*`) take an
...
use hjkl_buffer::{Buffer, Position, Wrap, is_keyword_char, wrap};
...
pub fn move_right_to_end(buf: &mut Buffer, count: usize) {
    let cursor = buf.cursor();
    let line = buf.line(cursor.row).unwrap_or("");
...
pub fn move_line_start(buf: &mut Buffer) {
    let row = buf.cursor().row;
    buf.set_cursor(Position::new(row, 0));
}

/// `^` — first non-blank column. On a blank line it lands on 0.
pub fn move_first_non_blank(buf: &mut Buffer) {
    let row = buf.cursor().row;
    let col = buf
        .line(row)
        .unwrap_or("")
        ...
        .unwrap_or(0);
    buf.set_cursor(Position::new(row, col));
}

/// `$` — last char on the row. Empty rows stay at column 0.
pub fn move_line_end(buf: &mut Buffer) {
    let row = buf.cursor().row;
    let col = last_col(buf.line(row).unwrap_or(""));
    buf.set_cursor(Position::new(row, col));
}

/// `g_` — last non-blank char on the row. Empty / all-blank rows
/// stay at column 0.
pub fn move_last_non_blank(buf: &mut Buffer) {
    let row = buf.cursor().row;
...
pub fn move_paragraph_prev(buf: &mut Buffer, count: usize) {
    let mut row = buf.cursor().row;
    for _ in 0..count.max(1) {
        ...
        while r > 0 && buf.line(r).is_some_and(|l| l.is_empty()) {
            r -= 1;
        }
        while r > 0 && buf.line(r).is_some_and(|l| !l.is_empty()) {
            r -= 1;
...
pub fn move_paragraph_next(buf: &mut Buffer, count: usize) {
    ...
    for _ in 0..count.max(1) {
        ...
        while r < last && buf.line(r).is_some_and(|l| l.is_empty()) {
            r += 1;
        }
        while r < last && buf.line(r).is_some_and(|l| !l.is_empty()) {
            r += 1;
...
pub fn move_up(buf: &mut Buffer, count: usize, sticky_col: &mut Option<usize>) {
    move_vertical(buf, -(count.max(1) as isize), sticky_col);
}

/// `j` — `count` rows down. See [`move_up`] for sticky ownership.
pub fn move_down(buf: &mut Buffer, count: usize, sticky_col: &mut Option<usize>) {
    move_vertical(buf, count.max(1) as isize, sticky_col);
}
...
pub fn move_top(buf: &mut Buffer) {
...
pub fn move_bottom(buf: &mut Buffer, count: usize) {
    let last = buf.row_count().saturating_sub(1);
...
pub fn move_word_fwd(buf: &mut Buffer, big: bool, count: usize, iskeyword: &str) {
    for _ in 0..count.max(1) {
        let from = buf.cursor();
...
pub fn move_word_back(buf: &mut Buffer, big: bool, count: usize, iskeyword: &str) {
    for _ in 0..count.max(1) {
        let from = buf.cursor();
...
pub fn move_word_end(buf: &mut Buffer, big: bool, count: usize, iskeyword: &str) {
    for _ in 0..count.max(1) {
        let from = buf.cursor();
...
pub fn move_word_end_back(buf: &mut Buffer, big: bool, count: usize, iskeyword: &str) {
    for _ in 0..count.max(1) {
        let from = buf.cursor();
        match prev_word_end(buf, from, big, iskeyword) {
            Some(p) => buf.set_cursor(p),
            None => break,
        }
    }
}
...
pub fn match_bracket(buf: &mut Buffer) -> bool {
    let cursor = buf.cursor();
    let line = match buf.line(cursor.row) {
        Some(l) => l,
        None => return false,
    };
    ...
    if forward {
        ...
        loop {
            let chars: Vec<char> = buf.line(r).unwrap_or("").chars().collect();
            while c < chars.len() {
                let here = chars[c];
                if here == open {
                    depth += 1;
                    ...
                    depth -= 1;
                    if depth == 0 {
                        buf.set_cursor(Position::new(r, c));
                        return true;
                    }
                }
                c += 1;
            }
            if r + 1 >= buf.row_count() {
                return false;
            }
            r += 1;
            c = 0;
        ...
        loop {
            ...
            while c >= 0 {
                ...
                if here == close {
                    ...
                    depth -= 1;
                    if depth == 0 {
                        buf.set_cursor(Position::new(r, c as usize));
                        return true;
                    }
                }
                c -= 1;
            ...
            r -= 1;
            c = buf.line(r).unwrap_or("").chars().count() as isize - 1;
        }
    }
}
...
/// Returns `true` when the cursor moved.
pub fn find_char_on_line(buf: &mut Buffer, ch: char, forward: bool, till: bool) -> bool {
    let cursor = buf.cursor();
    let line = match buf.line(cursor.row) {
        Some(l) => l,
        None => return false,
    ...
    match target_col {
        Some(col) => {
            buf.set_cursor(Position::new(cursor.row, col));
            true
        }
        None => false,
    }
...
fn move_screen_vertical(buf: &mut Buffer, delta: isize, sticky_col: &mut Option<usize>) {
    ...
    *sticky_col = Some(buf.cursor().col);
}
...
fn step_screen(buf: &mut Buffer, down: bool, visual_col: usize) -> bool {
    ...
    if down {
        if seg_idx + 1 < segs.len() {
            ...
            buf.set_cursor(Position::new(cursor.row, target));
            return true;
        }
        let Some(next_row) = buf.next_visible_row(cursor.row) else {
            return false;
        };
        ...
        let Some(prev_row) = buf.prev_visible_row(cursor.row) else {
            return false;
        };
...
fn move_vertical(buf: &mut Buffer, delta: isize, sticky_col: &mut Option<usize>) {
    let cursor = buf.cursor();
    ...
    *sticky_col = Some(want);
    ...
    if delta < 0 {
        for _ in 0..(-delta) as usize {
            match buf.prev_visible_row(target_row) {
                Some(r) => target_row = r,
                None => break,
            }
        }
        ...
        for _ in 0..delta as usize {
            match buf.next_visible_row(target_row) {
                Some(r) => target_row = r,
                None => break,
            }
        }
    }
    let line = buf.line(target_row).unwrap_or("");
    let max_col = last_col(line);
    let target_col = want.min(max_col);
    buf.set_cursor(Position::new(target_row, target_col));
}
...
fn next_word_start(buf: &Buffer, from: Position, big: bool, iskeyword: &str) -> Option<Position> {
...
fn prev_word_start(buf: &Buffer, from: Position, big: bool, iskeyword: &str) -> Option<Position> {
...
fn prev_word_end(buf: &Buffer, from: Position, big: bool, iskeyword: &str) -> Option<Position> {
    let mut cur = step_back(buf, from)?;
    loop {
...
fn char_kind_or_space(buf: &Buffer, pos: Position, big: bool, iskeyword: &str) -> CharKind {
    char_at(buf, pos)
...
fn next_char_kind_in_row(buf: &Buffer, pos: Position, big: bool, iskeyword: &str) -> CharKind {
...
fn next_word_end(buf: &Buffer, from: Position, big: bool, iskeyword: &str) -> Option<Position> {
...
mod tests {
    ...
    fn motions_module_compiles_against_concrete_buffer() {
        ...
        // 0.1.0 plans to lift the bound to `B: Cursor + Query`.
        let mut b = Buffer::from_str("hello");
